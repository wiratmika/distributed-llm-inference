...
SERVICE_NAME="distributed-llm-worker-${worker_id}"
...
[Unit]
Description=Distributed LLM Worker ${worker_id}
...
Environment=NEXT_NODE_URL=$NEXT_NODE_URL
ExecStart=/usr/local/bin/poetry run uvicorn inference.worker:app --host 0.0.0.0 --port $APP_PORT
Restart=always
RestartSec=5
