...
Unlike earlier planning notes, the implemented and benchmarked configuration in this repository uses GPT-2 Medium, node counts \{1,2,4\}, prompt lengths \{32,128,512\}, and concurrency levels \{1,2,3\}. This change was driven by deployment constraints on the available university cloud account: high-memory-bandwidth instance types were not accessible, and GPT-2 XL runs repeatedly timed out despite large timeout settings. GPT-2 Medium was selected to preserve the original experimental structure while keeping runs feasible and reproducible in the accessible environment.

\section{Research Questions}
The evaluation is organized around the following research questions:

\begin{itemize}
\item \textbf{RQ1 (Scaling under single-client load):} How do latency and TTFT change as node count increases from 1 to 2 to 4?
\item \textbf{RQ2 (Time breakdown):} What fraction of end-to-end latency is attributable to network transfer versus model compute?
\item \textbf{RQ3 (Concurrency behavior):} Does higher request concurrency improve system-level throughput enough to offset distributed overhead?
\item \textbf{RQ4 (Input-length sensitivity):} How does prompt length influence the communication-to-compute ratio in distributed runs?
\item \textbf{RQ5 (Memory effects):} How does peak per-node memory change across different partitions?
\end{itemize}
...
The serving stack contains one gateway and multiple worker processes. The gateway accepts /generate requests, tokenizes prompts, and performs autoregressive decoding one token at a time. For each decoding step, it forwards token IDs (first stage) or hidden activations (later stages) to rank-0 worker, which recursively forwards outputs through subsequent workers until the last stage returns logits.
...
\end{itemize}
All runs use GPT-2 Medium, generation length 50, warmup run, and repeated measurements written to JSON.

\section{Evaluation}
\subsection{Platform and Setup}
Experiments are deployed on Google Cloud VMs (Ubuntu 22.04, machine type c4d-highmem-2) with one gateway and seven worker instances to realize 1/2/4-node logical pipelines. Inference uses CPU only.
...
Table~\ref{tab:latency_c1} summarizes median single-client latency and TTFT. Increasing nodes consistently increases latency for all prompt lengths.

\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{charts/1-latency-node-count.jpg}
\caption{Latency vs. node count with one line per input length (32, 128, 512), including run-level standard deviation error bars.}
...
\caption{TTFT vs. node count with one line per input length (32, 128, 512), including run-level standard deviation error bars.}
...
Relative to one node, two-node latency increases by 20.5\% (32-token), 27.3\% (128-token), and 24.9\% (512-token). Four-node latency increases by 60.2\%, 77.1\%, and 75.3\%, respectively. Figures~\ref{fig:latency_node_count} and~\ref{fig:ttft_node_count} visualize these trends. Under this implementation, added communication dominates reduced per-stage compute.

To make run-level stability explicit, Table~\ref{tab:raw_variability} reports mean and standard deviation per configuration.
...
Median network-time share of wall-clock latency increases sharply with node count:
\begin{itemize}
\item 1 node: 17.2--22.5\%
\item 2 nodes: 29.7--36.0\%
\item 4 nodes: 45.0--51.6\%
\end{itemize}
Conversely, compute share decreases from roughly 68--77\% (1 node) to 39--48\% (4 nodes). This quantitatively supports the communication-bound diagnosis for multi-node runs.

\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{charts/3-time-share-breakdown.jpg}
\caption{Stacked wall-clock time-share decomposition (compute, serialization, network, residual) for each \{input length, node count\} configuration at concurrency 1.}
\label{fig:time_share_breakdown}
\end{figure}

Table~\ref{tab:raw_share} integrates the raw decomposition values used for this claim.
...
\caption{Throughput vs. concurrency at input length 128, with one line per node count.}
\label{fig:throughput_concurrency}
\end{figure}
Table~\ref{tab:raw_tput_rel} presents throughput deltas relative to the 1-node baseline, showing that the deficit shrinks with concurrency but remains negative.
