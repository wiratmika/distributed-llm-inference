...
\item \textbf{Memory effects}: Does peak per-node RSS decrease proportionally with layer count as sharding increases, and what fixed overheads prevent linear scaling?
\end{itemize}
...
\caption{Grouped wall-clock time-share decomposition (compute, serialization, network, residual) for each input length and node count configuration at concurrency 1.}
...
Peak RSS per node does not decrease monotonically with more shards. At 4 nodes, rank-0 RSS remains near 2,400 MB despite hosting only 6 transformer blocks, compared to 2,500 MB for the single-node run hosting all 24 blocks. We attribute this to two compounding factors. First, each worker process incurs a fixed Python/PyTorch runtime baseline of approximately 300--400 MB regardless of layer count, which dominates per-process memory when few layers are assigned. Second, the current implementation loads the full embedding table and language model head on every worker; these components account for roughly 150 MB in GPT-2 Medium and are unnecessary on non-boundary ranks. Together these two sources can plausibly account for the observed $\sim$300 MB gap between measured and analytically expected per-node weight memory at 4 nodes. A controlled ablation---stripping embeddings from non-boundary workers and measuring the RSS delta---would confirm this attribution, but is left for future work.

\section{Conclusion}
This project demonstrates an end-to-end distributed LLM inference prototype using layer-wise pipeline partitioning over HTTP in a CPU-only environment. The implementation successfully supports 1/2/4-node sharding, detailed telemetry, and reproducible benchmarking. Communication and serialization overhead dominate benefits from parallel stage compute: single-request latency increases with node count across all prompt lengths, higher concurrency narrows but does not reverse the throughput gap, and per-node memory does not decrease proportionally with layer count due to fixed Python/PyTorch runtime overhead and duplicated boundary components.

Critically, we did not achieve the originally hoped throughput scaling with additional nodes under this setup. The study should be interpreted as a constrained but practical baseline rather than a final verdict on distributed inference, since unavailable high-bandwidth instance classes and timeout pressure prevented the originally intended larger-model evaluation. At larger model scales, the fixed per-process overhead identified here would represent a smaller fraction of total per-node memory, making it possible to cleanly test whether proportional memory reduction is achievable in practice.

Future work should focus on replacing synchronous HTTP forwarding with asynchronous messaging so that pipeline stages can overlap, reducing transport overhead via binary protocols or tensor compression, and re-running the same protocol on larger models and stronger hardware to test whether conclusions change at greater scale.
